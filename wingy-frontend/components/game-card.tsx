...
export function GameCard({ game, isSelected, onClick }: GameCardProps) {
  return (
    <button
      onClick={onClick}
      className={`p-2.5 sm:p-3 rounded-lg font-medium transition-all duration-200 border relative group min-h-[3rem] sm:min-h-[3.5rem] flex items-center justify-center ${
        isSelected
    ...
    >
      <span className="relative z-10 text-xs sm:text-sm text-center break-words hyphens-auto line-clamp-2">{game.name}</span>
      {isSelected && (
        <>
          ...
          <Check className="absolute top-1 right-1 w-3 h-3 sm:w-3.5 sm:h-3.5" />
        </>
      )}
    </button>
  )
